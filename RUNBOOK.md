...
The runner also waits for `datasets status` to report `ready` before pushing any
kernel that mounts it. That is cheap insurance, not the fix for anything observed.

**The real trap is the mount path.** Kaggle mounts sources at:

```
/kaggle/input/datasets/<owner>/<slug>/...
/kaggle/input/kernels/<owner>/<slug>/...
```

**not** `/kaggle/input/<slug>/...`, which is the flat layout most examples and
older notebooks show. A glob written for the flat layout matches nothing and
fails with `expected exactly 1 pyproject.toml ... found []` — which reads exactly
like "the dataset isn't attached" and sends you looking in the wrong place. It
took out two prep sessions before a throwaway kernel that just listed
`/kaggle/input` settled it.

`find_one` now globs `/kaggle/input/**/<slug>/**/<file>`, which spans both
layouts, and prints what *is* mounted when it fails.

**Kaggle's output browser does not render nested directories.** A finished prep
shows `filtered/`, `unfiltered/`, and `lexicons/` as "empty" even when they are
not. Download the output and look before concluding anything failed.
