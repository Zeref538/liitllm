...
def prep_cells(smoke: bool = False):
  ...
  return [
    ...
    (f"BAKEOFF_N = {2_000 if smoke else 50_000}\n"
     f"TARGET = {2_000_000 if smoke else 1_000_000_000}   # tokens per corpus\n"
...
# where it stopped.
# One long session per run instead of many short ones.
#
# Sizing, from the pilot's measured 0.527 s/step at 8 layers, scaled by 9/8 for
# the extra layer: ~0.593 s/step, plus eval overhead (~50 s every 2000 steps)
# gives ~0.618 s/step all-in. 60,000 steps therefore lands at ~10.4 hours —
# inside the 9-11h window and clear of Kaggle's 12-hour wall.
#
# The point of one session is that nothing has to hand off. A multi-part chain
# needs a machine awake to push part N+1 when part N finishes; a single session
# runs entirely server-side, so the laptop can sleep through it.
TOTAL_STEPS = 60000
PART_STEPS = 60000     # the whole run in one go
MAX_HOURS = 11.0       # hard backstop: stop cleanly before the 12h wall

# Parts 2 and 3 are CONTINUATIONS, not scheduled work. They exist because a
# single-session run has nothing to resume from if it dies — there is no
# "previous part" holding a checkpoint. Each continuation mounts its predecessor
# and targets the same final step, so it picks up wherever the last one stopped
# and finishes the run. Normally only part 1 ever runs.
MAX_PARTS = 3

# Four runs: both corpora, two seeds each. The second seed is not optional if the
# ablation is going to claim anything — with one run per arm, a gap between the
# arms cannot be distinguished from run-to-run noise, and evaluate.py reports
...
def train_cells(arm: str, part: int, n_parts: int):
    config = ARMS[arm]
    stop_at = TOTAL_STEPS - 1   # every part targets the END of the run
    return [
